...
function f!(F,x)
	...
	P_u_US_US   = P_s_ji(x[4]^2,0.0,p_u_US_US,θ)
	...
	P_u_RoW_RoW = P_s_ji(x[6]^2,0.0,p_u_RoW_RoW,θ)
	P_u_US_RoW  = P_s_ji(x[4]^2,0.0,p_u_US_RoW,θ)
	...
	P_d_US_US  = P_s_ji(x[3]^2,0.0,p_d_US_US,σ)
	...
	c_US_US    = c_ji(x[1]^2,L_US,x[7]^2,0.0,p_d_US_US,σ,P_d_US)
	...
	P_d_RoW_RoW = P_s_ji(x[5]^2,0.0,p_d_RoW_RoW,σ)
	P_d_US_RoW  = P_s_ji(x[3]^2,0.0,p_d_US_RoW,σ)
	P_d_RoW     = P_s_i(P_d_US_RoW,P_d_RoW_RoW,σ)
	c_RoW_RoW   = c_ji(x[2]^2,L_RoW,x[8]^2,0.0,p_d_RoW_RoW,σ,P_d_RoW)

	c_US_RoW  = c_ji(x[2]^2,L_RoW,x[8]^2,0.0,p_d_US_RoW,σ,P_d_RoW)
	...
	# Tax revenue
	F[7] = x[7]^2 - (- ν*x[3]^2*c_US_US  *p_d_US_US   - ν*x[3]^2*x[4]^2*x_US_US  *p_u_US_US)   - (t_d_RoW_US*x[5]^2*c_RoW_US*p_d_RoW_US + t_u_RoW_US*x[3]^2*x[6]^2*x_RoW_US*p_u_RoW_US - ν*x[3]^2*c_US_RoW*p_d_US_RoW - ν*x[5]^2*x[4]^2*x_US_RoW*p_u_US_RoW)
	F[8] = x[8]^2 - (- ν*x[5]^2*c_RoW_RoW*p_d_RoW_RoW - ν*x[5]^2*x[6]^2*x_RoW_RoW*p_u_RoW_RoW) - (- ν*x[5]^2*c_RoW_US*p_d_RoW_US - ν*x[3]^2*x[6]^2*x_RoW_US*p_u_RoW_US)
end
